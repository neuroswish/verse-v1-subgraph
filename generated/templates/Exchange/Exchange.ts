...
export class Exchange extends ethereum.SmartContract {
  ...
  hyperobject(): Address {
    let result = super.call("hyperobject", "hyperobject():(address)", []);

    return result[0].toAddress();
  }

  try_hyperobject(): ethereum.CallResult<Address> {
    let result = super.tryCall("hyperobject", "hyperobject():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
